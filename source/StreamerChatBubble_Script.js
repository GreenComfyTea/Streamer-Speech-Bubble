...
window.addEventListener('onWidgetLoad', async obj => {
  ...
  conditionalMainClass('custom-message-background-color', FieldData.useCustomMessageBackgroundColor)
  conditionalMainClass('custom-message-text-color', FieldData.useCustomMessageTextColor)
  conditionalMainClass('enable-message-text-shadow', FieldData.enableMessageTextShadow)

  
  conditionalMainClass('disable-userbox', FieldData.disableUserbox)
  conditionalMainClass('custom-userbox-background-color', FieldData.useCustomUserboxBackgroundColor)
  conditionalMainClass('custom-username-color', FieldData.useCustomUsernameColor)
  conditionalMainClass('enable-username-text-shadow', FieldData.enableUsernameTextShadow)
...
function loadFieldData(data) {
  ...
  processFieldData(
    ...
    'emoteOnly',
    'highlightOnly',
    'useCustomMessageBackgroundColor',
    'useCustomMessageTextColor',
    'enableMessageTextShadow',
    'disableUserbox',
    'useCustomUserboxBackgroundColor',
    'useCustomUsernameColor',
    'enableUsernameTextShadow',
    'previewMode',
    'largeEmotes',
    'showBadges',
    'showUsername',
...
async function onMessage(event, testMessage = false) {
  ...
  const tmiSentTs = event.data.tags["tmi-sent-ts"];
  ...
  const elementData = {
    parsedText,
    name: displayName,
    timestamp: formatTmiTimestamp(tmiSentTs),
    emoteSize,
    messageType,
    msgId,
    userId,
  ...
  window.setTimeout(_ => {
    ...
    let maxWidth =
      FieldData.fixedWidth
        ? FieldData.maxWidth
        : $(`${currentMessage} .message-wrapper`).width() + 1
   
    let minWidth = 0;
    if(!FieldData.disableUserbox) {
       minWidth = $(`${currentMessage} .username`).outerWidth()
    }
    ...
    if (FieldData.positionMode !== 'list') {
      ...
      let xMax = Math.max(minWidth, maxWidth) + 45
...
function BubbleComponent(props) {
  ...
  if (!FieldData.showUsername || FieldData.disableUserbox) {
	name = ''
  }

  if (!FieldData.disableUserbox) {
    if (FieldData.pronounsMode === 'suffix' && pronouns) {
      if (FieldData.showUsername) {
        name = `${name} (${pronouns})`
      }
      else {
        name = `${pronouns}`
    ...
    if (FieldData.showTimestamp) {
       if (FieldData.showUsername || (FieldData.pronounsMode === 'suffix' && pronouns)) {
        name = `${name} - ${timestamp}`
      }
      else {
        name = `${timestamp}`
  ...
  const darkerColor = tinycolor
    .mix(
      FieldData.useCustomUserboxBackgroundColor ? FieldData.userboxBackgroundColor : color,
      'black',
      25,
    )
    .toString()
  ...
  if (FieldData.showBadges && !FieldData.disableUserbox) {
    usernameChildren = BadgesComponent(badges)
  }
  if (FieldData.pronounsMode === 'badge' && pronouns && !FieldData.disableUserbox) {
    usernameChildren.push(PronounsBadgeComponent(pronouns))
  }
  usernameChildren.push(name)

  const usernameProps = {}
  if (!FieldData.useCustomUsernameColor) {
    usernameProps.style = {
      color: isDark
...
//    Helper Functions
// ---------------------
function formatTmiTimestamp(timestamp) {
  let locale = FieldData.timestampLocale
  if (FieldData.timestampLocale === '') {
    locale = 'en-GB'
  }
  
  let timezone = FieldData.timestampTimezone
  if (FieldData.timestampTimezone === '') {
    timezone = 'Etc/UTC'
  }
  
  return new Date(parseInt(timestamp)).toLocaleString(locale, {timeZone: timezone}).slice(-FieldData.timestampSlice);
}
...
function parse(text, emotes) {
  ...
  const parsedText = textObjs.reduce((acc, textObj, index) => {
    ...
    if (emoteData.type === 'twitch') {
      emoteData.urls['1'] = `https://static-cdn.jtvnw.net/emoticons/v2/${emoteData.id}/default/dark/1.0`
      emoteData.urls['2'] = `https://static-cdn.jtvnw.net/emoticons/v2/${emoteData.id}/default/dark/2.0`
      emoteData.urls['4'] = `https://static-cdn.jtvnw.net/emoticons/v2/${emoteData.id}/default/dark/3.0`
    }
